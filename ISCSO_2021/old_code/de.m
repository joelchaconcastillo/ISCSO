...
for i = 1:N
    ...
    [Weight,  Const_Vio_Stress,  Const_Vio_Disp] = ISCSO_2021(Population(i,1:NSizing_variables),  0);
...
for gen = 1:(maxeval/N)
    ...
    for i = 1:N
       ...
       [Weight,  Const_Vio_Stress,  Const_Vio_Disp] = ISCSO_2021(Trial(1:NSizing_variables),  0);
       fTrial = [Weight,  Const_Vio_Stress,  Const_Vio_Disp];
       fitnessF = F(i,1)+p1*F(i,2).^2 + p2*F(i,3).^2;
       if fitnessF < fbest
	       fbest = fitnessF
       	       vBest = fTrial;
	end
       fitnesstrial = fTrial(1)+p1*fTrial(2).^2 + p2*fTrial(3).^2;
