...
size_dim = [ 37*ones(1,NSizing_variables), (3500+25000)*ones(1,NShape_variables)];
Flag = 0;
maxeval = 200000;
...
while feval < maxeval
   ...
   if f_current < f_best_local
      [current, f_current, w, s, d ] = best_variable(current, f_current, w, s, d, idx, lower(idx), upper(idx), stress_penalization, displacement_penalization, NSizing_variables, NShape_variables);
      feval = feval + (min(upper(idx), current(idx)+5) -  max(lower(idx), current(idx)-5))
      best_local = current;
