...
stress_penalization = 1;%1000000000;
displacement_penalization = 1;%1000000000;
...
while feval < maxeval
   stress_penalization = 10000*(feval/maxeval);
   displacement_penalization= 10000*(feval/maxeval);
   current = best_local;
   for i =1:3
      idx = randi([1,dim]);
   ...
   if f_current < f_best_local
	...
	rr = [best_local, best_solution_global];
        save('ILS_history_local_best_less', 'rr', '-ascii', '-append');
...
%	feval = feval +1;
   ...
   if f_best_local < f_best_global
    ...
    save('ILS_history_less', 'row_v', '-ascii', '-append');
   end
   feval = feval+1;
end
