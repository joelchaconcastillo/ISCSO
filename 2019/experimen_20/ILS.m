...
while feval < maxeval
   ...
   if f_current < f_best_local
	...
	rr = [best_local, best_solution_global];
        save('ILS_history_local_best_30update_5', 'rr', '-ascii', '-append');
	idx = randi([1,dim]);
	for i =idx:min(idx+5, dim)
           best_local(i) = randi([lower(i), upper(i)]);
	end
   ...
   if f_best_local < f_best_global
    ...
    save('ILS_history_30_5', 'row_v', '-ascii', '-append');
   end
   feval = feval+1;
end
